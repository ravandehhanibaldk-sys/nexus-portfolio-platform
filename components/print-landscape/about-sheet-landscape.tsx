        ...
        <div className="flex-1 grid grid-cols-2 gap-[14mm]">
          <div>
            {intro.paragraphs.map((p, i) => (
              <p key={i} className="pt-meta font-body leading-snug text-ink/85 mb-2">
                {p.en}
              </p>
            ))}
            {ideaSite.heading ? (
              <h2 className="pt-subhead font-display text-ink mt-4 mb-2">{ideaSite.heading.en}</h2>
            ) : null}
            {ideaSite.paragraphs.map((p, i) => (
              <p key={i} className="pt-meta font-body leading-snug text-ink/85 mb-1.5">
                {p.en}
              </p>
            ))}
            {ideaSite.pullQuote ? (
              <p className="pt-subhead font-display text-ink leading-snug my-3 pl-4 border-l-2 border-accent">
          ...
          <div className="flex flex-col">
            {howIWork.heading ? (
              <h2 className="pt-subhead font-display text-ink mb-3">{howIWork.heading.en}</h2>
            ) : null}
            {howIWork.paragraphs.map((p, i) => (
              <p key={i} className="pt-meta font-body leading-relaxed text-ink/85 mb-3">
                {p.en}
              </p>
            ))}
                ...
                {cycle.cycle!.en.split(" → ").flatMap((word, wi, words) => {
                  ...
                  const nodes = [
                    <span
                      ...
                      className={
                        isInput
                          ? "pt-micro font-body text-neutral tracking-[0.08em] uppercase"
                          : "pt-meta font-body text-ink tracking-[0.08em] uppercase"
                      }
                    >
                      {word}
                    </span>,
                  ...
                  if (wi < words.length - 1) {
                    nodes.push(
                      <span key={`a-${wi}`} className="pt-micro font-body text-neutral/40">
                        →
                      </span>
                    );
                  }
