...
- **Date:** 2026-08-22
- **Status:** Accepted
- **Purpose:** Decide whether the all-landscape redesign (`app/print/test-full-landscape-redesign`, `components/print-landscape/*`, print v3) replaces the mixed portrait/landscape pipeline (`app/print/portfolio`, `components/print/*`) as the production PDF, per the PDF track's three current priorities (alignment review, orientation decision, `@page` CSS fix).
- **Why selected:** Two inputs. (1) **Alignment review completed across all 15 landscape pages** (previously only 3/15 had been spot-checked at the rendered-PDF level, via `pdf-export/qa-v3-pdf-view/`; the remaining 12 were rendered and reviewed in this pass, `pdf-export/qa-v3-pdf-view-full/`). No misalignment found on any page. The documented v3 fix (`red-sun-comparison-sheet-landscape.tsx`, fixed subtitle min-height) is confirmed working — all three B-2/C-1/D columns align exactly despite D's subtitle wrapping to 2 lines where B-2/C-1 wrap to 1. The `justify-between` narrative-column distribution (sheets 3 and 9) is consistent and intentional, not a defect. (2) **Direct comparison against Draft 02 (`pdf-export/hanibal-ravandeh-portfolio-draft-02.pdf`)** shows the mixed-orientation approach has a real, structural problem this content doesn't have in landscape: the Red Sun comparison page (3 side-by-side proposal columns) leaves roughly the bottom 45% of the portrait page empty, because the content is inherently landscape-shaped (wide comparison grids, wide site plans, wide environmental diagrams, wide architectural photography); the Villa Efe project-opening hero photo is likewise cropped awkwardly to fit a portrait frame. The landscape format is a structurally better fit for this specific portfolio's content, not merely a stylistic preference.
- **Alternatives considered:** Keep Draft 02 (mixed orientation) as production and backport the alignment fix to `components/print/red-sun-comparison-sheet.tsx` — rejected; it would fix the alignment bug but not the underlying page-shape mismatch (the dead-space and crop problems are orientation problems, not alignment problems, and are absent in the landscape version). Maintain both pipelines in parallel — rejected as unnecessary maintenance surface once the landscape version is verified complete; Draft 01/02 and their generating code are left in place as historical artifacts, not deleted.
- **Consequences / tradeoffs:** `app/print/portfolio`, `components/print/*` (portrait sheets), `app/print/print.css`, and `scripts/export-pdf.mjs` are now legacy — not deleted (nothing here asked for deletion), but no longer the production path. Their own `@page { size: A4 }` rule (ambiguous for the landscape-tagged sheets in that mixed pipeline, worked around only by the two-pass `page.pdf({ landscape })` JS option in `export-pdf.mjs`) is consequently moot and was **not** fixed as part of this pass — fixing it would mean investing further in a pipeline this ADR retires. If the mixed-orientation approach is ever revived, that `@page` rule needs the same treatment as ADR-009's landscape fix (see `export-portfolio-landscape.mjs`) before it can be called production-correct.
- **Potential future risks:** None identified for the landscape pipeline itself. The reserved Reflection beat (`content/projects/*.ts`, `beats.reflection`) is not yet rendered anywhere in the landscape print sheets — out of scope for this ADR, tracked separately.
